...
void LLViewerEventRecorder::updateMouseEventInfo(S32 local_x, S32 local_y, S32 global_x, S32 global_y, const std::string& name) 
{
	if (!mLogEvents) return; // <alchemy/>

	LLView* target_view = LLUI::resolvePath(LLUI::getRootView(), mXui);
	if (!target_view)
	{
	...
	LL_DEBUGS() << "LLViewerEventRecorder::updateMouseEventInfo b4 updatemouseeventinfo - local_x|global x   " << this->local_x << " " << this->global_x << "local/global y " << this->local_y << " " << this->global_y << " mname: " << name << " xui: " << mXui << LL_ENDL;
	...
	if (!name.empty() && name != "/" && !mXui.empty())
	{
		//xui=std::string("/")+mName+xui; 
		//xui=mName+xui; 
		mXui = name; // TODO review confirm we never call with partial path - also cAN REMOVE CHECK FOR "" - ON OTHER HAND IT'S PRETTY HARMLESS
	}

	LL_DEBUGS() << "LLViewerEventRecorder::updateMouseEventInfo after updatemouseeventinfo - local_x|global x   " << this->local_x << " " << this->global_x << "local/global y " << this->local_y << " " << this->global_y << " mname: " << name << " xui: " << mXui << LL_ENDL;
}
...
void LLViewerEventRecorder::update_xui(const std::string& xui) 
{
	if (!mLogEvents) return; // <alchemy/>

	if (!xui.empty() && mXui.empty()) {
...
void LLViewerEventRecorder::logMouseEvent(const std::string& button_state, const std::string& button_name)
{
	if (!mLogEvents) return; // <alchemy/>
